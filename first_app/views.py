...
def pre_welcome(request):
    return render(request,'first_app/api_test.html')
    ...
    try:
        if 'data' in request.POST:
            ...
            if sorted(ds.keys())==sorted(['currency','start_date','end_date']):
                ...
                # return render_to_response('first_app/result_page.html',locals())
                # return JsonResponse(transfer)
                return HttpResponse(json.dumps(transfer), content_type='application/json')
            else:
                return HttpResponse(status=403)
                # return HttpResponseRedirect(request,"first_app/api_test.html")
